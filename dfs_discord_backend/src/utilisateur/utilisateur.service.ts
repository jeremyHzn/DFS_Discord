...
export class UtilisateurService {
  ...
  async creerSalon(nom: String, idServeur:number): Promise<Utilisateur> {
    //on ajoute le salon a la liste des salons du serveur
    const utilisateur = await this.utilisateurModel.findOne({ nom: nom },
      { $addToSet: { salons: idServeur } }, // $addToSet évite les duplications
      { new: true }, // Retourner le document mis à jour
    );
